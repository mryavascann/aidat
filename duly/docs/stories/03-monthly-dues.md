...
- The solo demo prepares all three wallets with 20 test USDC each. **My account** shows the active account and each demo wallet's on-chain balance. Existing demos can use **Get test USDC**. Funding alone does not put money in the treasury or dues ledger. The existing demo remains three simulated apartments controlled by one tester.
...
4. `demo-funds`: the earlier bank-backed manager-only funding endpoint remains available for its saved recovery records. The current interface no longer uses it to prepare demo wallets.

Demo preparation uses the disposable wallets' Friendbot XLM and a direct Stellar
Testnet XLM → configured Circle USDC path payment. It targets 20 USDC per wallet,
caps the spend at 100 test XLM with 5% quote headroom, and saves each plan and
signed envelope before submission. Repeating preparation resumes the same
per-wallet intent; it does not replenish a completed grant after spending it.
No bank order is created. Available testnet liquidity is still required.
See [Stellar path payments](https://developers.stellar.org/docs/build/guides/transactions/path-payments).
...
`npm --prefix web run test:demo-wallets` creates an isolated live testnet demo,
checks all three 20-USDC balances and funding replay, contributes 5 USDC, then
checks balance refresh, reload and eight account-dialog language/theme/viewport
accessibility scenarios. Its private browser state stays in ignored test results.
